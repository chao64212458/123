  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <el-input v-model="query.articleType" clearable placeholder="文章类型" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        <label class="el-form-item-label">首页推荐标志 0:关闭 1:开启</label>
        <el-input v-model="query.topFlg" clearable placeholder="首页推荐标志 0:关闭 1:开启" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" /> -->
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="1280px" @opened="onDialogOpen">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="120px">
          ...
          <el-form-item label="发布时间">
            <el-date-picker v-model="form.articleDate" type="datetime" style="width: 370px;" />
          </el-form-item>
          <el-form-item label="文章来源">
            <el-input v-model="form.articleFrom" style="width: 370px;" />
          </el-form-item>
          <el-form-item label="缩略图文件名">
            ...
            />
            <label class="el-form-item-label">※图片尺寸要求：409 x 273</label>
          </el-form-item>
...
<script>
import crudTProject from '@/api/fenghui/project'
...
let image = ''
const defaultForm = { articleId: null, title: null, introduction: '', content: '', thumbnail: null, topFlg: 0, captureFlg: 0, searchType: 2, updateUser: null, updateTime: null, tips: null, createUser: null, createTime: null, articleType: 0, articleDate: null, articleFrom: null }
export default {
  name: 'TProject',
  components: { pagination, crudOperation, rrOperation, udOperation, DateRangePicker, myUpload },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  // dicts: ['article_type', 'article_top_type'],
  cruds() {
    return CRUD({ title: '重点项目', url: 'api/project', idField: 'articleId', sort: 'articleId,desc', crudMethod: { ...crudTProject }})
  },
  data() {
    return {
      show: false,
